...
void reallocate_vector(dbds_vector* v, size_t new_capacity, size_t data_size) {
  v->data = realloc(v->data, new_capacity * data_size);
  v->capacity = new_capacity;
}

void grow_vector(dbds_vector* v, size_t data_size) {
  reallocate_vector(v, calculate_capacity_growth(v->capacity), data_size);
}

void shrink_vector(dbds_vector* v, size_t data_size) {
  reallocate_vector(v, v->size, data_size);
}
...
DBDS_EXPORT void* dbds_vector_push_back(dbds_vector* v, const void* value,
  ...
  if (vector_needs_grow(v)) grow_vector(v, data_size);
  void* last = (unsigned char*)v->data + v->size * data_size;
  v->size++;
  return memcpy(last, value, data_size);
}
...
DBDS_EXPORT void* dbds_vector_get(const dbds_vector* v, size_t i,
  ...
  return (unsigned char*)v->data + i * data_size;
}
